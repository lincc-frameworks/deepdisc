...
class WeightedRedshiftPDFCasROIHeads(CascadeROIHeads):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.zbins = torch.tensor(zbins)
        self.weights = torch.tensor(weights).to(self.dummy_param.device)
    ...
    def _forward_redshift(self, features, instances):
        ...
        if self.training:
            ...
            self.zbins = self.zbins.to(fcs.device)
            self.weights = self.weights.to(fcs.device)
