...
class AugRedshiftPDFCasROIHeads(CascadeROIHeads):
    ...
    def _forward_redshift(self, features, instances):
        ...
        if self.training:
            zhigh = gt_redshifts>2.0
            if thigh.any():
                faug = cat([torch.flip(x, [0]).unsqueeze(0) for x in features])

                features_aug = cat([features,faug])
                gt_redshifts_aug = cat([gt_redshifts,gt_redshifts[zhigh]])                

                diff = prediction - truefull

            features_aug = nn.Flatten()(features_aug)
            ebvs = cat([x.gt_ebv for x in instances])
            ebvs_aug = cat([ebvs,ebvs[zhigh]])
            features = torch.cat((features_aug, ebvs_aug.unsqueeze(1)), dim=-1)
            
            fcs = self.redshift_fc(features)
            pdfs = self.output_pdf(fcs)

            #gt_redshifts = cat([x.gt_redshift for x in instances])
            nlls = -pdfs.log_prob(gt_redshifts_aug) * self.zloss_factor
...
class EBVRedshiftPDFCasROIHeads(CascadeROIHeads):
    """CascadeROIHead with added redshift pdf capability.  Follows the detectron2 CascadeROIHead class init, except for
...
class NewRedshiftPDFCasROIHeads(CascadeROIHeads):
    """CascadeROIHead with added redshift pdf capability.  Follows the detectron2 CascadeROIHead class init, except for
