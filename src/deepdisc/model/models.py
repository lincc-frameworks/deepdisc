...
class WeightedRedshiftPDFCasROIHeadsGold(CascadeROIHeads):
    ...
    def _forward_redshift(self, features, instances, targets=None):
        ...
        if self.training:
            ...
            #limit redshift range to avoid indexing error
            sample_bins = torch.clamp(sample_bins,max=len(self.zbins)-1)
    ...
    def forward(self, images, features, proposals, targets=None):
        del images
        if self.training:
            print('proposals ', len(proposals[0]), 'targets', len(targets[0]))
...
class OldRedshiftPDFCasROIHeadsGold(CascadeROIHeads):
    """CascadeROIHead with added redshift pdf capability.  Follows the detectron2 CascadeROIHead class init, except for
